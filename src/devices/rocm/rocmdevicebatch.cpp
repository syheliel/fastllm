...
namespace fastllm {
    void RocmSplitBatchOp::Reshape(const std::string &opType, const fastllm::DataDict &datas,
    ...
    void RocmSplitBatchOp::Run(const std::string &opType, const fastllm::DataDict &datas,
                              const fastllm::FloatDict &floatParams, const fastllm::IntDict &intParams) {
        Data &input = *(datas.find("input")->second);
        Data **outputs = (Data**)(datas.find("output")->second);
        int axis = intParams.find("axis") != intParams.end() ? intParams.find("axis")->second : -1;
        ...
        FastllmRocmSplitBatch(input, outputs, axis);
    }

    void RocmCatBatchOp::Reshape(const std::string &opType, const fastllm::DataDict &datas,
                                const fastllm::FloatDict &floatParams, const fastllm::IntDict &intParams) {
    ...
    void RocmCatBatchOp::Run(const std::string &opType, const fastllm::DataDict &datas,
                             const fastllm::FloatDict &floatParams, const fastllm::IntDict &intParams) {
        Data **inputs = (Data**)(datas.find("input")->second);
        Data &output = *(datas.find("output")->second);
        int axis = intParams.find("axis") != intParams.end() ? intParams.find("axis")->second : -1;
        ...
        int part = intParams.find("input___batch")->second;
        output.Allocate();
        FastllmRocmCatBatch(inputs, output, axis);
    }

    void RocmMulBatchOp::Run(const std::string &opType, const fastllm::DataDict &datas,
                             const fastllm::FloatDict &floatParams, const fastllm::IntDict &intParams) {
        Data **inputs = (Data**)(datas.find("input")->second);
        Data **outputs = (Data**)(datas.find("output")->second);
        ...
        FastllmRocmMulBatch(inputs, v, batch, outputs);
    }

    void RocmMatMulBatchOp::Reshape(const std::string &opType, const fastllm::DataDict &datas,
                                         const fastllm::FloatDict &floatParams, const fastllm::IntDict &intParams) {
        ...
        if (input0s[0]->dims.size() == 3 && input1s[0]->dims.size() == 3) {
            ...
            fastllm::BaseOperator *op = (fastllm::BaseOperator*)(new RocmMatMulOp());
    ...
    void RocmMatMulBatchOp::Run(const std::string &opType, const fastllm::DataDict &datas,
                                     const fastllm::FloatDict &floatParams, const fastllm::IntDict &intParams) {
        int batch = intParams.find("input0___batch")->second;
        Data **input0s = (Data**)(datas.find("input0")->second);
        Data **input1s = (Data**)(datas.find("input1")->second);
        ...
        for (int i = 0; i < batch; i++) {
            ...
            for (int b = 0; b < batch0; b++) {
                i0s.push_back((float*)input0.rocmData + b * input0Spatial);
                i1s.push_back((float*)input1.rocmData + b * input1Spatial);
                os.push_back((float*)output.rocmData + b * outputSpatial);
                ns.push_back(n);
                ms.push_back(m);
                ks.push_back(k);
        ...
        FastllmRocmBatchMatMulBatch(i0s.data(), i1s.data(), os.data(),
                                    ns.data(), ms.data(), ks.data(),
                                    i0Strides.data(), i1Strides.data(), alpha, (int)i0s.size());
    }

    void RocmMatMulTransBBatchOp::Reshape(const std::string &opType, const fastllm::DataDict &datas,
                                          const fastllm::FloatDict &floatParams, const fastllm::IntDict &intParams) {
        int batch = intParams.find("input0___batch")->second;
        Data **input0s = ((Data **) datas.find("input0")->second);
        Data **input1s = ((Data **) datas.find("input1")->second);
        ...
        if (input0s[0]->dims.size() == 3 && input1s[0]->dims.size() == 3) {
            ...
            fastllm::BaseOperator *op = (fastllm::BaseOperator *) (new RocmMatMulTransBOp());
            DataDict tempDatas = datas;
            for (int i = 0; i < batch; i++) {
                tempDatas["input0"] = ((Data **) datas.find("input0")->second)[i];
                tempDatas["input1"] = ((Data **) datas.find("input1")->second)[i];
    ...
    void RocmMatMulTransBBatchOp::Run(const std::string &opType, const fastllm::DataDict &datas,
                                      const fastllm::FloatDict &floatParams, const fastllm::IntDict &intParams) {
        int batch = intParams.find("input0___batch")->second;
        Data **input0s = (Data**)(datas.find("input0")->second);
        Data **input1s = (Data**)(datas.find("input1")->second);
        ...
        FastllmRocmBatchMatMulTransBBatch(i0s.data(), i1s.data(), os.data(),
                                         ns.data(), ms.data(), ks.data(),
                                         i0Strides.data(), i1Strides.data(), alpha, (int)i0s.size());
    }

    void RocmSoftmaxBatchOp::Run(const std::string &opType, const fastllm::DataDict &datas,
                                 const fastllm::FloatDict &floatParams, const fastllm::IntDict &intParams) {
        int axis = intParams.find("axis") != intParams.end() ? intParams.find("axis")->second : -1;
        int batch = intParams.find("input___batch")->second;
        Data **inputs = (Data**)(datas.find("input")->second);
        ...
        FastllmRocmSoftmaxBatch(inputs, outputs, axis, batch);
    }

    void RocmCatDirectBatchOp::Run(const std::string &opType, const fastllm::DataDict &datas,
                                   const fastllm::FloatDict &floatParams, const fastllm::IntDict &intParams) {
        Data **input0s = (Data**)(datas.find("input0")->second);
        Data **input1s = (Data**)(datas.find("input1")->second);
        int batch = intParams.find("input0___batch")->second;
        ...
        for (int b = 0; b < batch; b++) {
            ...
            if (input0.dims.size() == 0) {
                ...
                dsts.push_back(input0.rocmData);
                dpitchs.push_back(input0Stride * unitSize);
                srcs.push_back(input1.rocmData);
            ...
            dsts.push_back((uint8_t *) input0.rocmData + oldDims[axis] * inner * unitSize);
            dpitchs.push_back(input0Stride * unitSize);
            srcs.push_back(input1.rocmData);
            spitchs.push_back(input1Stride * unitSize);
            widths.push_back(input1.dims[axis] * inner * unitSize);
        ...
        FastllmRocmMemcpy2DDeviceToDeviceBatch(dsts.data(), dpitchs.data(), srcs.data(),
                                               spitchs.data(), widths.data(), heights.data(), dsts.size());
    }

    void RocmAttentionBatchOp::Reshape(const std::string &opType, const fastllm::DataDict &datas,
                                      const fastllm::FloatDict &floatParams, const fastllm::IntDict &intParams) {
    ...
    void RocmAttentionBatchOp::Run(const std::string &opType, const fastllm::DataDict &datas,
                                  const fastllm::FloatDict &floatParams, const fastllm::IntDict &intParams) {
        int batch = intParams.find("q___batch")->second;
        Data **qs = (Data**)(datas.find("q")->second);
        Data **ks = (Data**)(datas.find("k")->second);
        ...
        FastllmRocmAttentionBatch(qs, ks, vs, masks, outputs,
